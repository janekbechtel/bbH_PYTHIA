...
#ifdef vlq_cxx
vlq::vlq(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
     TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("EWK_BX_8TeV__bp_MQ160GeV_WQ20.1GeV.root");
     //     TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("EWK_BX_13TeV__bp_MQ160GeV_WQ20.1GeV.root");
      if (!f || !f->IsOpen()) {
	f = new TFile("EWK_BX_8TeV__bp_MQ160GeV_WQ20.1GeV.root");
	//	f = new TFile("EWK_BX_13TeV__bp_MQ160GeV_WQ20.1GeV.root");
      }
      f->GetObject("t1",tree);
...
Int_t vlq::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t vlq::LoadTree(Long64_t entry)
{
...
Double_t vlq::deltaPhi(Double_t phi1, Double_t phi2)
{
...
Double_t vlq::deltaEta(Double_t eta1, Double_t eta2)
{
  Double_t deta = fabs(eta1 - eta2);
  return deta;
}

Double_t vlq::deltaR(Double_t eta1, Double_t phi1, Double_t eta2, Double_t phi2)
{
