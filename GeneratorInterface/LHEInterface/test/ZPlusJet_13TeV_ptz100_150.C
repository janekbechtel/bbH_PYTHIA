...
void Draw()
{
  ...
  TFile* file = new TFile("ptz100_150.root");
  ...
  hptj_div_ptz_barrel->SetMaximum(0.070);
  ...
  TH1F * hptj_div_ptz_forward = new TH1F("hptj_div_ptz_forward", "ptj_div_ptz_forward", 200, 0., 2.);
  hptj_div_ptz_forward->Add(hptj_div_ptz_forwardp,hptj_div_ptz_forwardm,1.,1.);
...
//  Latex
  ...
  tex->SetLineWidth(2);
  tex->Draw();
  TLatex *tex = new TLatex(0.23,0.96,"#it{Preliminary}");
  tex->SetNDC();
  tex->SetTextFont(43);
  tex->SetTextSize(18);
  tex->SetLineWidth(2);
  ...
  TLatex *t = new TLatex();
  t->SetTextSize(0.05);
  t->DrawLatex(0.4,0.064,"Z(#mu#mu) + #geq 1 jet, p_{T}^{j}>25 GeV, |#eta|<4.7");
  t->SetTextSize(0.04);
  t->DrawLatex(0.15,0.057,"#bullet HLT_IsoMu24 or HLT_IsoTkMu24");
  t->DrawLatex(0.15,0.052,"#bullet p_{T}^{#mu1,2}>25, 20 GeV, |#eta^{#mu}|<2.1, 80<m_{#mu#mu}<100 GeV");
  t->DrawLatex(0.15,0.047,"#bullet 100<p_{T}^{Z}<150 GeV, #Delta#phi(Z-j1)>2.6, p_{T}^{miss}<40 GeV");
  t->SetTextSize(0.04);
  t->DrawLatex(0.9,0.040,"JEC Summer16_23Sep2016V4");
  t->DrawLatex(1.35,0.007,"j1 - leading p_{T} jet");
  c1->SaveAs("ZplusJet_13TeV_ptz100_150.pdf"); 
}
