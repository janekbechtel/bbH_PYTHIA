...
void Draw()
{
  ...
  leg->AddEntry(hetab1,"MG5_aMC@NLO ","L");
  leg->Draw();
  //  Latex
  ...
  leg->AddEntry(hetab2,"MG5_aMC@NLO ","L");
  leg->Draw();
  //  Latex
  TLatex *tex = new TLatex(0.15,0.96,"#bf{CMS}");
  tex->SetNDC();
  ...
  hetaj->SetLineWidth(3);
  hetaj->Draw("samehist");
  leg->AddEntry(hetaj,"MG5_aMC@NLO ","L");
  leg->Draw();
  //  Latex
  TLatex *tex = new TLatex(0.15,0.96,"#bf{CMS}");
  tex->SetNDC();
  ...
  TLatex *t = new TLatex();
  t->SetTextSize(0.05);
  t->DrawLatex(-4.0,0.11,"gg#rightarrowbbA, m_{A}=700 GeV; #geq 1b selection");
  c3->SaveAs("etaj_mA700.pdf"); 
